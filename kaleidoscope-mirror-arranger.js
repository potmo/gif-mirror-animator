...
export function* createReflectionsArrangement(settings, world_objects) {
    ...
    for (let x = 0; x < settings.input.mirrors.width; x++) {
        for (let y = 0; y < settings.input.mirrors.height; y++) {
            ...
            let pos = upperLeft
                        ...
                        .add(world_objects.mirror_board.heightVector.scale(y / settings.input.mirrors.height))
                        .add(vector(0,0,-settings.three_dee.mirror_thickness));

            let target = vector(midDir.x * 0.2 , midDir.y * 0.2, 4);
