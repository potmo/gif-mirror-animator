...
> `TODO: The abstract can be written in a simlper language`
...
> `TODO: mention George Rousse maybe?`

> `TODO: Some more examples`

> `TODO: maybe this needs to be a bit more in depth rather than just namedrops?`

> `TODO: The footnotes should contains figures of the referenced artworks`

> `TODO: Maybe talk about image making machines`
...
As will become apparent in just a few moments it is a lot easier to reproduce an image that contains few unique colours. This is usually referred to a _reduced palette_ in contrast to use all possible colours. The obvious drawback of using a reduced palette is that the fidelity of the image will also be reduced. Fortunately computer games had to work with the same limitations[^early_computers] for many years during the 70's, 80's and early 90's so a lot of research was put in to find clever ways to hide the effects of using a reduced palette.

In general there are two kinds of images that works well with a reduced palette. Either very noisy images with a lot of high-frequency details [^high-frequency-details]or solid colour fields as in cartoons. The reason noisy images works well is that when you reduce the colour palette the result will also be noisy and the image will be perceived as noisy both before and after. Low frequency details like shades and gradients tend to produce aliasing artefacts that is more difficult to hide with few colours. Cartoon style images doesn't contain shades or gradients and are produced with the constraint of a reduced palette in mind.

The image one wants to create generally is going to be low resolution given the fact that one will have to fabricate it later and an image with two million mirrors will be quite a project. One can also bear in mind each mirror will be very large in comparison to for example a computer screen.  that  it can be smart to take inspiration from digital images that are made to work being very small. The obvious example that comes to mind is application icons that are ment to be legible even at very small sizes. Another inspiration can be emojis or emoticons that are created to flow in text[^pixel-density] while being legible at those small sizes.
...
We have established that we want to produce images with a reduced palette. One way is of course to draw the images with a reduced palette from the start but sometimes it is desirable to use an already existing image and reduce it's palette. The naïve approach is to iterate over all pixels and then find the closest one in a predefined palette. Sometimes, if we are lucky, it's not easily detectable with the human eye. Two shades of red might be just a hair different but removing one and replacing it with the other might not make any noticeable difference[^noticeable]. At a first glance this seems like a reasonable approach but when digging a bit deeper it will become apparent that just defining what "closest" actually mean is very hard. Also, if you don't have a predefined palette but want a way to compute the palette with the maximum perceived similarity the problem becomes incredibly hard, to be precise NP-hard[^NP-hard]. In the technical literature this problem is referred to as _color space quantisation_[^quantisation].

This problem is computationally very hard but there exists a few good algorithms that can compute approximations. The standard color space quantisation algorithm that is used in for example Photoshop is called k-means clustering[^k-means clustering] and was invented in 1957. It basically finds <img src="svgs/55a049b8f161ae7cfeb0197d75aff967.svg?invert_in_darkmode" align=middle width=9.86687624999999pt height=14.15524440000002pt/> points so that the distance from all colours location in the colour space is minimised. Another common algorithm is _Median Cut_ that is used less these days because of its poor quality. K-means and Median Cut tends to produce washed out colours when reducing the palette to very few colours since the means in the regular rgb-space is gray.  A different, slightly better, but slow and resource intensive algorithm is _Hierarchical Agglomerative clustering_ or _Spatial Color Quantisation_ that were both published in 1994. I have got the best results with the _NeuQuant_ algorithm by Anthony Dekker[^dekker-1994] from 1994 that uses pretrained Kohonen neural networks to find the best palette and assign each pixel one colour from it.
...
> `TODO: About hexagonal pixel arrays: https://www.centeye.com/technology/hexagonal-pixel-arrays/`
