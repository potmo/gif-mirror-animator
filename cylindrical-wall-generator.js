...
export function getWall(settings) {
  ...
  const wall_position = vector(0,0,0)
                          ...
                          .add(wall_normal.scale(wall_width).scale(0.5).scale(-1)) // move away so that the leading edge is at offset point
  ...
  wall.worldNormalAtTextureCoord = (x, y) => {
    var rotation = x + 0.5 + settings.three_dee.wall_rotation_scalar + 0.25;
    return vector(1,0,0)
              .rotatedAroundY(Math.PI * 2  * rotation, vector(0,0,0))
              .normalized()
  }
